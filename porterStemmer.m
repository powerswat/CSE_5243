...
if k > 2
    ...
    x = step1ab(b, k, k0); if(length(x{1})==1);stem=x{1};return;end;
    %disp(sprintf('Steps 1A and B yield: %s', x{1}));
    x = step1c(x{1}, x{2}, k0); if(length(x{1})==1);stem=x{1};return;end;
    %disp(sprintf('Step 1C yields: %s', x{1}));
    x = step2(x{1}, x{2}, k0); if(length(x{1})==1);stem=x{1};return;end;
    %disp(sprintf('Step 2 yields: %s', x{1}));
    x = step3(x{1}, x{2}, k0); if(length(x{1})==1);stem=x{1};return;end;
    %disp(sprintf('Step 3 yields: %s', x{1}));
    x = step4(x{1}, x{2}, k0); if(length(x{1})==1);stem=x{1};return;end;
    %disp(sprintf('Step 4 yields: %s', x{1}));
    x = step5(x{1}, x{2}, k0); if(length(x{1})==1);stem=x{1};return;end;
